...
export default function TopResult({ result, type }: TopResultProps) {
  ...
  return (
    <div className="bg-neutral-900 rounded-lg p-6 hover:bg-neutral-800 cursor-pointer group transition-colors">
      <div className="flex items-end gap-4">
        <img
          src={image || "/placeholder.svg"}
          alt={title}
          className={`w-24 h-24 ${isTrack ? "rounded" : "rounded-full"} object-cover`}
        />
        <div className="flex-1 min-w-0">
          ...
          {isTrack && (
            <button
              onClick={handleClick}
              className="bg-green-500 hover:bg-green-400 text-black font-semibold px-8 py-3 rounded-full transition-colors opacity-0 group-hover:opacity-100"
            >
