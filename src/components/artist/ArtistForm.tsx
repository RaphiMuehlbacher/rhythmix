...
const artistFormSchema = z.object({
	...
	profilePicUrl: z.string().trim().optional().or(z.literal("")),
})
...
export default function ArtistForm({defaultValues}: ArtistFormProps) {
	...
	useEffect(() => {
		const url = form.getValues("profilePicUrl") || defaultValues.profilePicUrl
				...
				<form onSubmit={form.handleSubmit(submitHandler)} className="space-y-6">
					...
					<div className="relative w-32 h-32 mx-auto">
						{isEditing ? (
								...
								>
									<DropzoneContent>
										<img
												src={
														filePreview ||
														form.getValues("profilePicUrl") ||
														defaultValues.profilePicUrl || "https://icons.veryicon.com/png/o/miscellaneous/management-system-icon-library/account-24.png"
												}
												alt="Profile"
								...
								<img
										src={form.getValues("profilePicUrl") || defaultValues.profilePicUrl}
										alt="Profile"
										className="w-32 h-32 rounded-full object-cover"
								/>
						)}
					...
					{/* Hidden field for profilePicUrl */}
					<FormField
